...
本地默认使用 `permissions.db`。`bi_agent_permission_dimension` 定义启用且必配的权限维度，
`bi_agent_user_data_scope` 按 OA `loginusername + dimension_id + dimension_value` 保存授权值。
...
向 `bi_agent_permission_dimension` 和 `bi_agent_user_data_scope` 增加数据即可，不需要修改查询流程。
